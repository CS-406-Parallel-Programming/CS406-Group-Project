...
void wrapper(int *xadj, int *adj, int n,  int nov, int nnz){
  ...
  int totalChunck = (nov + CHUNK_SIZE -1) /CHUNK_SIZE;
  ...
  #pragma omp parallel num_threads(PARALEL_THREAD_COUNT)
  {
    ...
    // create for output
    gpuErrchk(cudaMalloc((void**)&output_d, (CHUNK_SIZE) * sizeof(int)));
    cudaEventCreate(&start);
    cudaEventRecord(start, 0);

    while (currentChunk != totalChunck)//do work
    {
      printf("threadId %d chunk %d \n", threadId,currentChunk);
      int thisChunk;
      ...
      int novStart   = thisChunk * CHUNK_SIZE;
      int novEnd =  (thisChunk+1)* CHUNK_SIZE;
